...
c
       wtimer = dble(nwtimer(1)) + dble(nwtimer(2)) * 1.0e-6
c
...
c         print *,'timcmn(1),dwtimer == ',timcmn(1),dwtimer
          case default
...
c      data error check
c
       if((myid.lt.master).or.(myid.gt.numblk)) then
...
c
       ...
       if((myid.gt.master).and.(myid.le.numblk)) then
       timeriblk(1) = timio(1)
       ...
       call sendtime(master,myblk,timeriblk,9)
       end if
c
...
c
       ...
       do iblk = 1,numblk
       call recvtime(master,iblk,timeriblk,9)
       do i = 1,9
