      ...
      DIMENSION R(K,M),Y(N,M),Z(K,K),R1(3),NC(4)
...
C ........                       (J=1,N & I=1,M)
      IF (M.LE.1) THEN
       WRITE(*,*) 'Number of Nodes = ',M
       WRITE(*,*) 'but expected more than 1'
       STOP 2222
      ENDIF
      DO 1 I=1,K
      L=NC(1)
...
20    CONTINUE
      ...
      IF ((NK.EQ.1).AND.(K.EQ.2)) THEN
       Z(1,2) = -Z(2,1)
       Z(2,2) = Z(1,1)
       GOTO 60
      ENDIF 

      IF ((NK.EQ.1).AND.(K.EQ.3)) THEN
       IF (Z(2,1)**2.GT.0.4) THEN
        Z(1,2) = -Z(2,1)
        Z(2,2) = Z(1,1)
        Z(3,2) = 0.0D0
       ELSE
...
c300   CONTINUE
      ...
      SUBROUTINE invm(N,RC,DET)
      IMPLICIT REAL*8 (A-H,O-Z)
      DIMENSION RC(N,N),A(10,20)
      M=N*2
      ERR=DET
      DET = 1.0
      DO 10 I=1,N
...
8     FORMAT(1X,6F12.3)
      ...
      IMPLICIT REAL*8 (A-H,O-Z)
      DIMENSION  T(NA,NT) , A(NA,NA) , TA(1000) , TAT(NT,NT)
...
C.......CUMPUTE TF = F
   ...
   10 TF(L) = F(L)
      ...
      SUBROUTINE ZLG (NT,T,F)
      IMPLICIT REAL*8 (A-H,O-Z)
      DIMENSION  T(NT,NT) , F(NT), TF(3)
C.......CUMPUTE TF = T(t)*F
      DO 20 I = 1 , NT
      CC = 0.0
      DO 10 L = 1 , NT
...
C.......CUMPUTE TF = T(t)*F
   ...
   30 F(L) = TF(L)
      ...
      SUBROUTINE ttt(Etr,EMTR)
      IMPLICIT REAL*8 (A-H,O-Z)
      DIMENSION ETR(3,3),EMTR(6,6)
c.....ETR  l(1), m(1), n(1)
c          l(2), m(2), n(2)
c          l(3), m(3), n(3)
c****	 x'=etr*x
c.....EMTR
c.....1-3,1-6
c    l(1)^2  m(1)^2     n(1)^2   2*m(1)*n(1) 2*n(1)*l(1) 2*l(1)*m(1) 
c    l(2)^2  m(2)^2     n(2)^2   2*m(2)*n(2) 2*n(2)*l(2) 2*l(2)*m(2) 
c    l(3)^2  m(3)^2     n(3)^2   2*m(3)*n(3) 2*n(3)*l(3) 2*l(3)*m(3)
c....4-6,1-3
c    l(2)*l(3)  m(2)*m(3) n(2)*n(3)
c    l(3)*l(1)  m(3)*m(1) n(3)*n(1)  
c    l(1)*l(2)  m(1)*m(2) n(1)*n(2)  
c.....4-6,4-6
c      m(2)*n(3)+m(3)*n(2)  n(2)*l(3)+n(3)*l(2) l(2)*m(3)+l(3)*m(2)  
c      m(3)*n(1)+m(1)*n(3)  n(3)*l(1)+n(1)*l(3) l(3)*m(1)+l(1)*m(3)  
c      m(1)*n(2)+m(2)*n(1)  n(1)*l(2)+n(2)*l(1) l(1)*m(2)+l(2)*m(1)  
c..............
c****  tao'=emtr*tao

      do i=1,3
	   do j=1,3
	   emtr(j,i)=etr(j,i)*etr(j,i)
	   enddo
      enddo
      do j=1,3
	   emtr(j,4)=2.0*etr(j,2)*etr(j,3)
	   emtr(j,5)=2.0*etr(j,3)*etr(j,1)
         emtr(j,6)=2.0*etr(j,1)*etr(j,2)
	   emtr(4,j)=etr(2,j)*etr(3,j)
	   emtr(5,j)=etr(3,j)*etr(1,j)
         emtr(6,j)=etr(1,j)*etr(2,j)
      enddo

	   emtr(4,4)=etr(2,2)*etr(3,3)+etr(3,2)*etr(2,3)
	   emtr(4,5)=etr(2,3)*etr(3,1)+etr(3,3)*etr(2,1)
	   emtr(4,6)=etr(2,1)*etr(3,2)+etr(3,1)*etr(2,2)
	   emtr(5,4)=etr(3,2)*etr(1,3)+etr(1,2)*etr(3,3)
	   emtr(5,5)=etr(3,3)*etr(1,1)+etr(1,3)*etr(3,1)
	   emtr(5,6)=etr(3,1)*etr(1,2)+etr(1,1)*etr(3,2)
	   emtr(6,4)=etr(1,2)*etr(2,3)+etr(2,2)*etr(1,3)
	   emtr(6,5)=etr(1,3)*etr(2,1)+etr(2,3)*etr(1,1)
	   emtr(6,6)=etr(1,1)*etr(2,2)+etr(2,1)*etr(1,2)
...
c      write(*,'(1x,3e10.4)') ((t(i,j),j=1,3),i=1,3)
...
c      write(*,*) (dfdx(i),i=1,m)
      ...
      subroutine nzdmbsadd(neq,neq1,maxa,na,jdiag,jdiagr,jdiagaz,l,lm)
      implicit real*8 (a-h,o-z)
      dimension na(maxa),jdiag(neq),jdiagr(neq),lm(l),jdiagaz(neq1)
...
c
            if(noyes.eq.0) then
               ...
               if((nk.lt.jdiagaz(ii)).or.(nk.gt.(jdiagaz(ii+1)-1)))
...
300   continue
      ...
      subroutine nzdmbsorder(neq,neq1,maxa,na,jdiag,jdiagr,jdiagaz,l,lm)
      implicit real*8 (a-h,o-z)
      dimension na(maxa),jdiag(neq),jdiagr(neq),lm(l),jdiagaz(neq1)
      if (neq1.le.1) return
      neq = neq1 - 1
...
c
      ...
      subroutine dmbsgetdiag(neq,neq1,maxa,na,jdiag,jdiagr,jdiagaz,l,lm)
      implicit real*8 (a-h,o-z)
      dimension na(maxa),jdiag(neq),jdiagr(neq),lm(l),jdiagaz(neq1)
      if (neq1.le.1) return
      neq = neq1 - 1
      do i = 1,neq
         ...
         if (noyes.ne.1) then
           print *,'Get diag position error!noyes =',noyes
           print *,'Error from neqnum == ',i,'nl,nu == ',nl,nu
         endif
      end do
      return
      end
...
c        
          ...
          if(noyes.eq.0) then
            print *,' Na pointer dimension error! noyes = ',noyes
            print *,'jj,na == ',jj
            print *,(na(k),k=nl,nu)
          endif
          a(nup) = a(nup) + estif(j,i)
280     CONTINUE
300   CONTINUE
      RETURN
      END
